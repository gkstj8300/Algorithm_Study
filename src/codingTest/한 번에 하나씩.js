...
function solution(callAPI) {
  // 상태 관리 변수들
  let isFirstRequest = true;        // 첫 번째 요청 식별자  
  let latestToken = null;           // 순차 처리를 위한 최신 토큰
  let ongoingRequest = null;        // 현재 진행 중인 요청 추적

  // 첫 번째 요청: 토큰 없이 즉시 실행
  async function handleFirstRequest() {
    isFirstRequest = false;

    const requestPromise = callAPI(null);
  ...
  // 후속 요청: 이전 요청 완료 대기 후 실행
  async function handleSubsequentRequest() {
    // 핵심: 이전 요청이 완료될 때까지 대기
    if (ongoingRequest) {
      await ongoingRequest;
    }

    // 최신 토큰으로 새 요청 시작
    const requestPromise = callAPI(latestToken);
    ongoingRequest = requestPromise;
  ...
  return async function () {
    ...
    if (isFirstRequest) {
      ...
      return handleSubsequentRequest();
    }
  };
}